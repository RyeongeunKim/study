...
/*
	...
	- File클래스는 물리적으로 존재하는 파일을 삭제, 디렉터리 생성 등 파일 관리 작업을 지원하는 클래스
	...
	- 파일을 읽고 쓰는 작업을 하는 스트림통로 역할의 클래스는?
	  ...
	  FileReader,
	  FileWriter
	  등의 파일 입출력 스트림을 이용하고 File클래스는 이들 스트림객체를 생성할 때
...
public class FileClassExample {
	...
	public static void main(String[] args) {
		...
		try {
			...
			//File객체를 통해 접근하는 ReadMe.txt파일의 경로를 알려주는 메소드 호출
			System.out.println(f.getPath()); //C:\Temp\ReadMe.txt
			
			//File객체를 통해 접근하는 ReadMe.txt파일의 크기를 알려주는 메소드 호출
			System.out.println(f.length()); //0
			
			//현재 운영체제에서 사용하는 경로를 구분하는 경로 구분자를 알려주는 상수 호출
			//윈도우 운영체제의 구분자는 \
			//유닉스 운영체제 계열의 구분자는 /
			System.out.println(f.separator);
			
			//C:\\Temp폴더내부에 존재하지 않는 aaa폴더 생성하는 메소드 호출
			//폴더 생성에 성공하면 true반환하고 실패하면 false반환하는 메소드이다.
			boolean n = new File("C:\\Temp\\aaa").mkdir();
			//System.out.println(b);
			
			//C:\\Temp폴더내부에 있는 FileInfo.txt파일 삭제시키는 메소드 호출
			//파일 삭제에 성공하면 true반환하고 실패하면 false
			boolean b2 = new File("C:\\Temp", "FileInfo.txt").delete();
			//System.out.println(b2);
			
			//C:\\Temp폴더내부에 있는 파일 및 폴더 정보들을 문자열 배열에 담아 반환하는 메소드 호출
			String[] list = new File("C:\\Temp").list();
			
			//for반복문을 이용하여 C:\\Temp폴더 내부에 존재하는 파일 및 폴더 정보를 
			//위 String 배열에서 꺼내와서 출력
			for(int i=0; i<list.length; i++) {
				System.out.println("Temp폴더 내부의 정보 : "+list[i]);
			}
