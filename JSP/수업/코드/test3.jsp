...
<body>
  ...
  <%
    ...
    // => 내장객체는 JSP페이지가 서블릿클래스로 변환될 때 _jspService()메서드 안에서
    //    가장 먼저 선언됨.
  ...
  /*
  	=> 9가지 내장 객체 외워라!
   	...
   	[javax.servlet 패키지] - 8개
   	 ...
   	 out : 응답 페이지 전송을 위한 출력 스트림 객체(모니터)
   	 application : 해당 애플리케이션의 컨텍스트 정보를 저장한 객체(컨텍스트: 현재 프로젝트, pageContext보다 큰 개념)
 ...
 URL : <%=request.getRequestURL() %><br> <!-- 프로젝트명+아이피+포트번호 -->
 URI : <%=request.getRequestURI() %><br> <!-- 위에 꺼 뺀 나머지 위치 -->
 ...
 <hr>
 http 해더(user-agent) : <%=request.getHeader("user-agent") %><br>
 http 해더(accept-language) : <%=request.getHeader("accept-language") %><br>
 hrrp 해더(host) : <%=request.getHeader("host") %><br>
 http 해더(connection) : <%=request.getHeader("connection") %><br>
